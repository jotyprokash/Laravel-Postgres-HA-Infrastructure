...
## Overview
The architecture is designed to handle high-concurrency registration traffic while maintaining data integrity and high availability.
...
*   **Why?** PostgreSQL spawns a new OS process per connection. Very high client concurrency would quickly exhaust memory if every request opened a direct database connection. PgBouncer queues these requests and multiplexes them across a small, stable pool of backend connections (~200).
